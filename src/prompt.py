...
包含多个 Agent 的 prompt 构造函数：
- 编排 Agent (orchestrator): 分析场景结构和段落分类
- 对白专家 (dialogue): 提取并优化角色台词
- 动作专家 (action): 转换场景描写为舞台指示
- 导演 Agent (director): 全局预读，提取角色和剧情
- 审查 Agent (review): 检查剧本规范性
- 记忆专家 (memory_update): 提取角色更新和事件摘要
"""
...
def _format_character_registry(character_registry: dict[str, dict], with_header: bool = True) -> str:
